...
Client.prototype._onConnect = function() {
  ...
  parser.on('method', function(method) {
    ...
    if (self._cmd === CMD.CONNECT) {
      self._ready = true;
      self.emit('connect', self._sock, repInfo.bndAddr, repInfo.bndPort);
      self._sock.resume();
    } else if (self._cmd === CMD.BIND) {
      self.emit('bind', self._sock, repInfo.bndAddr, repInfo.bndPort);
      if (self._parser)
        self._parser.stop();
      self._parser = new Parser(self._sock, true);
...
Client.prototype.connect = function(options, cb) {
  ...
  if (!options.port && options.cmd === CMD.CONNECT)
  ...
  if (options && options.tls && typeof options.tls === 'object') {
    var realOptions = options.tls;
  } else {
    var realOptions = {};
  }
  realOptions.host = this._proxyhost,
  realOptions.port = this._proxyport,
  realOptions.localAddress = options.localAddress // TODO: remove?
  ...
  if (net.isIP(this._dstaddr) === 0 && this._localDNS) {
    dns.lookup(this._dstaddr, function(err, addr) {
      ...
      if (options && options.tls && typeof options.tls === 'object') {
        self._sock = tls.connect(realOptions);
      } else {
        self._sock.connect(realOptions);
      }
      self._sock.on('connect', function() {
        self._onConnect();
      }).on('error', function(err) {
      ...
      self._parser = new Parser(self._sock);
    });
  } else {
    if (options && options.tls && typeof options.tls === 'object') {
      self._sock = tls.connect(realOptions);
