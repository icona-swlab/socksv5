...
function proxySocket(socket, req, server) {
  if (server.logger)
    server.logger.debug('Connect to:'+JSON.stringify(req)+' from:'+socket.remoteAddress+':'+socket.remotePort);
...
function proxyBind(socket, req, server) {
  ...
  listenSock.on('error', function (e) {
    ...
    if (server.logger)
      server.logger.debug('Bind to:'+JSON.stringify(req)+' from:'+socket.remoteAddress+':'+socket.remotePort+' Listening on: '+address.address + ":" + address.port);
    writeResult(socket, address.address, address.port);
    timeOut = setTimeout(function() {
      var errbuf = new Buffer([0x05, REP.TTLEXPIRED]);
      if (server.logger)
        server.logger.info('Bind to: '+address.address + ":" + address.port+' end for timeout');
      socket.end(errbuf);
      listenSock.close();
    }, server.maxBindWait);
  }).on('connection', function(connectedSocket) {
    if (server.logger)
      server.logger.debug('Bind to:'+JSON.stringify(req)+' from:'+socket.remoteAddress+':'+socket.remotePort+'Connection from: '+connectedSocket.remoteAddress + ":" + connectedSocket.remotePort);
