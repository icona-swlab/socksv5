...
function proxyBind(socket, req, server) {
  ...
  listenSock.on('error', function (e) {
    ...
    if (server.logger)
      lserver.ogger.debug('Bind to:'+JSON.stringify(req)+' from:'+socket.remoteAddress+':'+socket.remotePort+'Connection from: '+connectedSocket.remoteAddress + ":" + connectedSocket.remotePort);
